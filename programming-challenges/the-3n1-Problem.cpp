...
int evenOperator(int n) {
  return n / 2;
}

int oddOperator(int n) {
  return (n * 3) + 1;
}

int main() {
  string line, token,startStr, endStr;
  int start, end, numCycles = 1;
  do {
    getline(cin, line);
    ...
    int n = 1, curNum = start;
    if (end < 1) return 0;
    while(curNum != 1) {
      if (curNum % 2 == 0) { //even number
        curNum = evenOperator(curNum);
      } else {
        curNum = oddOperator(curNum);
      }
     // cout << "a: "<< curNum << " " << numCycles <<"\n";
      numCycles++;
    }
  } while (!line.empty());

//  cout << start <<  " " << end << " " << numCycles << "!\n";
